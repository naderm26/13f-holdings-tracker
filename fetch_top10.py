...
# Top 10 most followed — must match ids in funds.json
TOP10_IDS = [
    "berkshire",
    "pershing",
    "scion",
...
# Only fetch top 10
top10_funds = [f for f in all_funds if f["id"] in TOP10_IDS]
print(f"Fetching {len(top10_funds)} funds: {[f['id'] for f in top10_funds]}")

for fund in top10_funds:
    fetch_fund(fund)

print("\nDone fetching top 10 funds.")
